...
def run_yolo_sign(model):
    ...
    if type(output) == int:
        Data = [None, None]
        return
    #Else Result is Best Name and percentage.
    else:
        output2 = output.cpu().numpy()
        percents = []
        
        for i in range(len(output2[:,7])):
            percents.append( output2[:,5][i] )

        max_index = np.argmax(percents)

        Data = [ classes[ int(output2[:,7][max_index]) ], output2[:,5][max_index] ]
